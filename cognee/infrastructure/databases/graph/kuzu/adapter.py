...
class KuzuAdapter(GraphDBInterface):
    ...
    # Frequency Weight Methods

    async def get_node_frequency_weights(self, node_ids: List[str]) -> Dict[str, float]:
        """Retrieve node frequency weights for multiple node ids. Returns only found node ids."""
        if not node_ids:
            return {}
        valid_node_ids = [node_id for node_id in node_ids if isinstance(node_id, str) and node_id]
        if not valid_node_ids:
    ...
    async def set_node_frequency_weights(
        ...
        """Persist node frequency weights for multiple node ids. Returns per-id update success."""
    ...
    async def get_edge_frequency_weights(self, edge_object_ids: List[str]) -> Dict[str, float]:
        """Retrieve edge frequency weights for multiple edge_object_ids. Returns only found edge ids."""
        if not edge_object_ids:
            return {}
        requested_ids = {eid for eid in edge_object_ids if isinstance(eid, str) and eid}
        if not requested_ids:
    ...
    async def set_edge_frequency_weights(
        ...
        """Persist edge frequency weights for multiple edge_object_ids. Returns per-id update success."""
